...
public class Door_L : GameElement
{
    ...
    public void Toggle()
    {
        if (ModifierManager.doorStuck && !stop)
        {
            ...
            {
                door_move.Play();
                Global.night.office.door_button_l.visible = !opening;
                stop = true;
                Global.night.doorClose_L = !closing;
            }
        }

        if (ModifierManager.oneDoorAtATime && (Global.night.doorClose_R || Global.night.office.door_R.closing))
        {
