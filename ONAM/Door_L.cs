    ...
    public void Toggle()
    {
        if (doorState == DoorState.JAMMED)
        {
            ...
            if (stuckCount < 1)
            {
                door_move.Play();
                doorState = jamState;
                Global.night.office.door_button_l.visible = doorState != DoorState.OPENING;
            }
        }

        if (ModifierManager.oneDoorAtATime && 
            !(Global.night.office.door_R.doorState == DoorState.OPENING && Global.night.office.door_R.stuckCount < 1)
                && Global.night.office.door_R.doorState != DoorState.OPEN)
        {
        ...
        {   
            ...
            else
            {
                ...
                if (stuckCount < 1) Global.night.office.door_button_l.visible = false;
            }
